...
function saveCompany(req, res) {
  const data = companiesModel(req.body)
  data
    .save()
    .then(
      companySaved =>
        // se salva la compañia
        companySaved
    )
    .then(companySaved => {
      ...
      return companiesModel
        .findOneAndUpdate(findBy, dataToUpdate)
        .then(result => {
...
function deleteCompany(req, res) {
  ...
  companiesModel
    ...
    .then(({ companyData, categoryIds }) =>
      ...
      serversModel
        .remove({ category: { $in: categoryIds } })
        .then(serversDeleted => ({ companyData, categoryIds }))
    )
    .then(({ companyData, categoryIds }) =>
      // se elimina todas las categorias que este en el arreglo
      categoriesModel
        .remove({ _id: { $in: categoryIds } })
        .then(categoriesDeleted => companyData)
    )
    .then(companyData =>
      // por ultimo se borra la compañia
      companiesModel.remove({ _id: companyData._id }).then(() => {
